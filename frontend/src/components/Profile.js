...
const Profile = ({ marketplace, nft, account,FRAX }) => {
  ...
  const sendClaim = async () => {
    ...
    console.log(price);
    const approve=await FRAX.approve(marketplace.address,priceInWei);
    await approve.wait();
      ...
      {messages.map((message, index) => (
        ...
        >
          <Card.Body>
            ...
            <Card.Text>
              <strong>Price:</strong> {(ethers.utils.formatEther(message.price).toString())} FRAX
            </Card.Text>
            <Card.Text>
